...
public class EmailService {
    ...
    public void sendMail(InvoiceDTO dto) throws MessagingException {
        ...
        context.setVariable("INVOICE", dto); // Set attribute for email.html (templates/common/email.html)

        String template = templateEngine.process("common/email", context); // Get path template email
        if (template!=null){
            try {
